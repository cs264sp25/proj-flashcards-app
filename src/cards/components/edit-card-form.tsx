...
const editCardFormSchema = createCardSchema;
type EditCardFormValues = z.infer<typeof editCardFormSchema>;

interface EditCardFormProps {
  onSubmit: (values: EditCardFormValues) => void;
  onCancel: () => void;
  initialValues: EditCardFormValues; // Expect initial values
  submitLabel?: string;
  // No deckId/cardId needed directly by the form logic, but could be added for future context
}
...
const EditCardForm: React.FC<EditCardFormProps> = ({
  ...
  const form = useForm<EditCardFormValues>({
    ...
    defaultValues: initialValues, // Use provided initial values
  });
  ...
  const getAvailableTasks = (fieldName: "front" | "back"): TaskType[] => {
    switch (fieldName) {
      case "front":
        // Could add edit-specific tasks later if needed
